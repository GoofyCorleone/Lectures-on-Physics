...
\begin{itemize}
	...
	\item \textbf{Modelado FDTD (Diferencia Finitas en el Dominio del Tiempo)}: Resuelve ecuaciones de Maxwell directamente, esencial en nanotecnología.
\end{itemize}
...
Las ondas electromagnéticas poseen inherentemente un carácter vectorial: en cada punto $\overrightarrow{r}$, el campo eléctrico puede exhibir un estado de polarización distinto. Esta propiedad se expresa matemáticamente como:
...
Aquí, $k = 2\pi/\lambda$ denota el número de onda, y $\lambda = c/(n\nu)$ corresponde a la longitud de onda en un medio con índice de refracción $n$. Esta formulación describe matemáticamente cómo cada componente monocromática del campo (caracterizada por su amplitud compleja $\psi$) obedece a una ecuación independiente de la frecuencia temporal.
...
\begin{equation}
	\oint_{\Sigma(V)} \left[ \psi(\vec{r}, \nu)\nabla g(\vec{r}) - g(\vec{r})\nabla \psi(\vec{r}, \nu) \right] \cdot d\vec{\sigma} = \int_{V} \psi(\vec{r}, \nu)\left(\nabla^2 + k^2\right)g(\vec{r})\, dV.
\end{equation}
...
\end{equation}
Curiosamente, podemos sumar una cantidad infinita de funciones de Green tales que la solución se mantiene invariante. Lo que podemos hacer es sumar una inifinidad de cargas imagen pero fuera del volumen de integración delimitado por la superficie $\Sigma(V)$ como se muestra en la imagen \ref{fig:greenimagenes}.
...
La  solución explícita para un medio homogéneo de la ecuaicón \ref{eq:Grennhomogenea} es:
...
\begin{itemize}
	\item $\psi = 0$ fuera de la apertura en $\Sigma$ (campo bloqueado),
	\item $\partial \psi/\partial n = 0$ en regiones opacas (derivada normal nula).
\end{itemize}
...
\begin{equation}
	\label{sol_Green}
	G(\vec{r}'; \vec{r}'') = \frac{e^{ik|\vec{r}'' - \vec{r}'_+|}}{|\vec{r}'' - \vec{r}'_+|} - \frac{e^{ik|\vec{r}'' - \vec{r}'_-|}}{|\vec{r}'' - \vec{r}'_-|}, 
\end{equation}

\noindent donde  
\begin{align}
	|\vec{r}'' - \vec{r}'_{-} | &= |\vec{r}'' - \vec{r}'_{+} | 
	\Rightarrow \quad |(x'' - x'_{+})| = |(x'' - x'_{-})|, \quad 
	|(y'' - y'_{+})| = |(y'' - y'_{-})|.
\end{align}
...
\begin{equation}
	\nabla G \cdot \hat{z}  = 2 \left[ik - \dfrac{1}{|\vec{1}-\vec{r}'|} \right]\dfrac{e^{ik|\vec{r}-\vec{r}'|}}{|\vec{r}-\vec{r}'|} \cos(\vec{r} - \vec{r}',\hat{z})
\end{equation}
...
Partiendo del campo 
\begin{align}
	U(\overrightarrow{r}') = \frac{ik}{2\pi}\int_{\Sigma} U(\overrightarrow{r}) \frac{e^{ik \left | \overrightarrow{r} - \overrightarrow{r}' \right |}}{\left | \overrightarrow{r} - \overrightarrow{r}' \right |} \cos(\overrightarrow{r} - \overrightarrow{r}', \widehat{z}) \mathrm{d}\sigma 
\end{align}
...
Es claro que en cuanto el factor $\dfrac{1}{|\vec{r}- \vec{r}'|}$, una expansión hasta el primer término de \ref{aprox_|r|} es suficiente, hecho que no resulta suficiente para el factor $e^{ik \left | \overrightarrow{r} - \overrightarrow{r}' \right |}$, puesto que este presenta un comportamiento oscilatorio en su dominio, por lo que ambos términos son requeridos. Estamos tomando además la consideración $\cos (\vec{r}-\vec{r}', \hat{z}) = \dfrac{z}{|\vec{r} - \vec{r} '|} \approx 1$. \\    
...
\begin{equation}
	\label{Fraunhofer}
	U(\vec{r}) = \dfrac{e^{ikz}}{\lambda z} \exp \left(\dfrac{ik}{2z} (x'^2 + y'^2)\right) \int u(r) exp\left( -\frac{ik}{z} (xx' + yy') \right) d\sigma
\end{equation}
...
Esto cobra mayor sentido una vez que anlizamos el efecto que tendría cada paso de la propagación. Es claro que, sin imponer de manera inicial aproximaciones de campo lejano propagamos çon la aproximación de Fresnel el campo en $\mathcal{A}$, hacia $\mathcal{F}$ obtendríamos 
...
Supongamos que tenemos un fenómeno completamente aleatorio como se muestra en la imagen \ref{fig:averages} o incluso, podemos suponer muchos más casos; la radiación electromagnética emitida por una bombilla incadescente que no se encuentra en equilibrio termodinámico, el número de autos en alguna ciudad capital a lo largo del día, el movimiento de un ión en un plasma o algo tan sencillo como lanzar una pelota al aire bajo condiciones no ideales.\\
...
\end{equation}
en donde podemos ver que, la similitud que hay entre $\ket{f_A}$  y $\ket{f_B}$ la podemos ver como el producto de $\hat{A}$ con $\hat{B}$ por la densidad de probabilidad de tener los estados $A$ y $B$, es decir, la simitud o cuán \textit{correlacionados} están dos variables la podemos ver como la integral del producto de las variables por la densidad de probabilidad mutua de estas.\\

Con esta analogía en mente podemos entender a la correlación como una primera medida de la similutd entre una o más variables a lo largo del espacio-tiempo. Si $x_1(\vec{r},t_1), x_2(\vec{r}',t_2)$ son dos variables aleatorias y si $p_2(x_2,t';x_1,t)$ es la densidad de probabilidad mutua entonces se define a la correlación entre $x_1,x_2$ como
...
En la figura \ref{fig:randomvar} podemos observar a una componente del campo elećtrico, representada por $E(\vec{r},t)$ vista a lo largo de varias observaciones o realizaciones. Esta al comportarse como variable aleatoria en cada realización tiene un comportamiento diferente a lo largo del tiempo en un punto $\vec{r}$, en general, acá hemos identado a $^{(i)}E_(\vec{r},t)$ con números enteros pero, en general, podría haber un conjuto no contable de posibles estados para el campo eléctrico. Si observamos la gráfica \ref{fig:randomvar} hay dos puntos temporales, $t,t'$ en los cuales aíslamos unas partes de estas oscilaciones aleatorias; podríamos preguntarnos cuán correlacionadas estas estas oscilaciones a lo largo de las realizaciones en este intervalor $t'-t$, o dicho de otra manera, qué tan \textit{coherentes} son las oscilaciones a lo largo de las realizaciones.\\
...
\end{equation}
en done estamos encontrando el promedio de ensamble de las componentes $\mathcal{E}$, las cuales son números complejos como ya hemos discutido en estas notas. Si tomamos a $\vec{r}=\vec{r}'$ tendremos a lo que se conoce como \textit{función de coherencia mutua}

\begin{equation}
	\Gamma(\vec{r'};t,t') = E \left\{ \mathcal{E}(\vec{r}',t) \mathcal{E}^*(\vec{r},t') \right\}.
\end{equation}

Podemos incluso ir un poco más allá incluyendo el concepto de \textit{estacionareidad estadística} pero hay que tener cuidado con esta puesto que hay varios tipos. Formalmente, un proceso estocástico $\{X_t\}$ se considera estacionario cuando sus propiedades estadísticas son invariantes bajo traslaciones temporales. Esta condición se manifiesta en diferentes grados:
...
Vamos a usar una hipótesis de estacionareidad fuerte, (aunque muchas veces basta con la estacionareidad débil). En este orden de ideas si $\tau = t-t'$ tenemos que 

\begin{equation}
	\Gamma(\vec{r}',t,t') = \Gamma(\vec{r}',t-t',t'-') = \Gamma(\vec{r}',\tau).
\end{equation}

Como podemos ver, la coherencia mutua en un punto $\vec{r}'$ bajo una hipótesis de estacionareidad da a lugar a una depenencia explícita en la diferencia de tiempos $\tau$ en el promedio de ensamble.\\
...
\begin{itemize}
	\item[a)] $\Gamma(\vec{r}',0) \geq 0$. Esto proviene de la definición puesto que $\Gamma(\vec{r}',0) = \braket{|\epsilon(\vec{r}')|^2}$, es decir, corresponden a la intensidad del campo eléctrico y solo puede ser cero cuando trivialmente en todo instante el campo es cero.
	...
	\item[c)] $|\Gamma(\vec{r},\tau) \leq \Gamma(\vec{r}',0)$. Esta propiedad proviene de la \textit{desiguadlad de Cauchy-Schwarz} 
	...
	\item[d)] $\Gamma(\vec{r}',\tau)$ es no negativa. Esto lo podemos ver a partir de al definición desde la siguiente desigualdad, un tanto obvia
	...
	\item[e)] Si el campo elećtrico es aleatorio, y estacionario y diferencialbe entonces podemos hallar uan velocidad $\xi (\vec{r}',t) = \dot{\mathcal{E}(\vec{r}',t)}$ y esta es estacionaria, aleatoria y de valor medio igual a cero, y su función de coherencia mutua es 
	
	\begin{equation}
		\Gamma_\xi(\vec{r}',\tau) = - \Ddot{\Gamma}_\mathcal{E}(\vec{r}',\tau).
	\end{equation}
	
	\item[f)] $\Gamma(\vec{r}',\tau)$ es continua. Esto es cierto incluso aún cuando existen discontinudiades de tipo salto.
\end{itemize}
...
Vamos a encontrar uno de los teoremas más importantes que existen en la teoría de la coherencia, el \textit{teorema de Wiener-Khinchin}. Fijémonos que las señales aleatorias estacionarias no tienen transformada de Fourier, por ende, podemos definir las siguietnes señales aleatorias del campo elećtrico
...
y definiremos a la potencial espectral para una realización $\omega$ como la magnitud al cuadrado de las transformadas de Fourier de los campos elećtricos \ref{eq:CasiWiener} , es decir
...
\begin{equation}
	S(\vec{r},\nu) = E\left\{^\omega S(\vec{r},t)\right\},
\end{equation}
...
\begin{align}
	...
	& S(\vec{r},t) = \lim_{T \to \infty} \frac{1}{T}  \int_{-T/2}^{T/2}\int_{-T/2}^{T/2} E \left\{ \mathcal{E}(\vec{r},t)\mathcal{E}^*(\vec{r'},t') \right\} e^{i2\pi\nu(t-t')}dtdt',\\
...
Y si aplicamos el operador D'Alembertiano $\square = \nabla^2 - \frac{1}{C^2}\frac{\partial^2}{\partial t^2}$ a la ecuación \ref{eq:EyFourier} tenemos que $\tilde{\mathbb{E}}(\vec{r},t)$ tendrá forma de una ecuación tipo Helmholtz, es decir
...
\end{equation}
nos lleva esto a que el campo elećtrico como señal analítica y compleja también obedece uan ecuación tipo onda
...
\begin{equation}\label{eq:IntegralEz}
	\mathcal{E}_z(\vec{r}_1,t) = \frac{1}{i\tilde{\lambda}} \int_{\Sigma} \mathcal{E}_0 \left(\vec{r}, t +\frac{|| \vec{r}_1 - \vec{r} ||}{V}\right) \frac{\cos\left(\hat{z} ; \vec{r}_1 - \vec{r}\right)}{|| \vec{r}_|-\vec{r} |||} d\sigma.
\end{equation}
...
\begin{equation}
	\Gamma_z(\vec{r}_1,\vec{r}2;\tau) = E \left\{ \mathcal{E}_z(\vec{r}_1,t) \mathcal{E}_z^*(\vec{r}_2,t-\tau) \right\},
\end{equation}
...
\end{equation}
ahora podremos hacer uan expansión en series de Taylor, la misma que se realizó en el capítulo de difracción. En este orden de ideas los siguientes términos se convierten en
...
\begin{equation}
	\Gamma_z(\vec{S_1},\vec{S}_2) = \frac{e^{i \frac{\pi}{\tilde{\lambda}z} (|\vec{S}_1|^2 - |\vec{S}_2|^2)}}{\tilde{\lambda}^2 z^2} \int_{\mathbb{R}^2} I_0(\vec{S})e^{-i \frac{2\pi}{\tilde{\lambda}z}\vec{S}\cdot (\vec{S}_1 - \vec{S}_2)}dS.
\end{equation} 
...
\end{equation}
lo que, básicamente, significa que las correlaciones, bajo todas las aproximaciones que hemos tomado, se propagan mediante una integral de Fourier, o lo que es lo mismo; el teorema de Van Citter-Zernike nos dice que la coherencia difracta, se propaga naturalmente como una integral de Difracción (ver figura \ref{fig:fouriergamma} ).\\
