...
\section{Interferometers}
    ...
    \subsection{Michelson and Morley interferometer}
        ...
        Let us now solve the instrument. The difficulty of the Michelson interferometer is not conceptual but bookkeeping, because the light is folded twice in each arm and a direct ray tracing forces us to carry two reflection points, four propagation legs and the two tilts all at the same time. The method of images removes the folding entirely. The idea is that a plane mirror does nothing to a wave except to make it appear to come from somewhere else, so that instead of following the light around the corner we may replace the mirror by the image of the source in it and let the light travel in a straight line. Applying this once for every reflection the beam actually suffers, each arm collapses into a single point source radiating freely into the output space, and the whole interferometer is reduced to the problem of two point sources that we already solved in the section on the interference of spherical waves. Everything that follows is the systematic execution of that idea.

        The tool we need is the reflection of a point in a plane. Let the plane pass through a point $\vec{P}$ and have unit normal $\hat{n}$, and let $\vec{X}$ be the point to be reflected. The quantity $\left(\vec{X}-\vec{P}\right)\cdot\hat{n}$ is the signed distance from $\vec{X}$ to the plane, positive on the side towards which $\hat{n}$ points, so that subtracting twice that distance along $\hat{n}$ carries the point across the plane and leaves it at the same distance on the other side. The image is therefore

        \begin{equation}\label{eq:MichReflectOp}
            \mathcal{R}\left(\vec{X}\right) = \vec{X} - 2\left[\left(\vec{X}-\vec{P}\right)\cdot\hat{n}\right]\hat{n},
        \end{equation}
        and it is worth pausing on why this simple operation is legitimate optics rather than a drawing trick. The map \ref{eq:MichReflectOp} is an isometry, since it preserves the distance between any two points, and it is an involution, since applying it twice returns the original point. The first property is the important one, because the optical path from the source to any point of the output space, measured along the true folded trajectory, is exactly equal to the straight line distance from the image to that same point. The image is not an approximation of the source, it is the source as far as every optical path length is concerned, and since interference depends on nothing but optical path lengths, the two descriptions are rigorously equivalent.

        We place the origin $O$ at the centre of the beamsplitting cube, take the $x$ axis along the first arm and the $y$ axis along the second, and read off the data of figure \ref{fig:MichelsonInterferometer}. The source sits at

        \begin{equation}\label{eq:MichSource}
            \mathcal{S} = \left(-a,\,0\right),
        \end{equation}
        the first mirror has its vertex on the $x$ axis at a distance $d+\ell_1$ from the origin and is tilted by $\theta$ away from the orientation perpendicular to its arm, so that its normal has turned by the same angle away from $\hat{x}$,

        \begin{equation}\label{eq:MichM1}
            \vec{P}_1 = \left(d+\ell_1,\,0\right), \qquad \hat{n}_1 = \left(\cos\theta,\,\sin\theta\right),
        \end{equation}
        while the second mirror has its vertex on the $y$ axis at a distance $d+\ell_2$ and is tilted by $\phi$ in the same sense, so that its normal has turned away from $\hat{y}$,

        \begin{equation}\label{eq:MichM2}
            \vec{P}_2 = \left(0,\,d+\ell_2\right), \qquad \hat{n}_2 = \left(-\sin\phi,\,\cos\phi\right).
        \end{equation}
        ...
        \begin{equation}\label{eq:MichBS}
            \vec{P}_B = \left(0,\,0\right), \qquad \hat{n}_B = \frac{1}{\sqrt{2}}\left(1,\,-1\right).
        \end{equation}
        To keep the expressions readable we abbreviate the two total arm lengths measured from the origin,

        \begin{equation}\label{eq:MichLengths}
            L_1 = d+\ell_1, \qquad L_2 = d+\ell_2,
        \end{equation}
        ...
        The first reflection is in $M_1$. We need the signed distance from the source to the plane of that mirror, and using \ref{eq:MichSource} and \ref{eq:MichM1},

        \begin{equation*}
            \mathcal{S}-\vec{P}_1 = \left(-a-L_1,\,0\right), \qquad
            \left(\mathcal{S}-\vec{P}_1\right)\cdot\hat{n}_1 = -\left(a+L_1\right)\cos\theta ,
        \end{equation*}
        a quantity that is negative, as it must be, since the source lies on the side opposite to the one the normal points to. Feeding this into \ref{eq:MichReflectOp},

        \begin{align*}
            & \mathcal{S}_1' = \left(-a,\,0\right) + 2\left(a+L_1\right)\cos\theta\,\left(\cos\theta,\,\sin\theta\right) = \\
            & \left(-a + 2\left(a+L_1\right)\cos^{2}\theta,\;\; 2\left(a+L_1\right)\sin\theta\cos\theta\right),
        \end{align*}
        and the two double angle identities $2\cos^{2}\theta = 1+\cos 2\theta$ and $2\sin\theta\cos\theta = \sin 2\theta$ turn this into

        \begin{align*}
            & \mathcal{S}_1' = \left({\color{red}-a} + \left(a+L_1\right) + \left(a+L_1\right)\cos 2\theta,\;\; \left(a+L_1\right)\sin 2\theta\right) = \\
            & \left(L_1 + \left(a+L_1\right)\cos 2\theta,\;\; \left(a+L_1\right)\sin 2\theta\right),
        \end{align*}
        ...
        \begin{equation}\label{eq:MichS1prime}
            \mathcal{S}_1' = \left(L_1 + \left(a+L_1\right)\cos 2\theta,\;\; \left(a+L_1\right)\sin 2\theta\right).
        \end{equation}

        It is worth stopping to read \ref{eq:MichS1prime} physically, because the tilt has entered doubled. This is the elementary and often forgotten fact that turning a mirror by an angle turns the reflected beam by twice that angle, and it appears here automatically, without our having invoked it, simply because the reflection operator acts twice on the component along the normal. Notice also that the distance from the origin to this image satisfies

        \begin{equation*}
            \left|\left|\mathcal{S}_1'\right|\right|^{2} = L_1^{2} + \left(a+L_1\right)^{2} + 2L_1\left(a+L_1\right)\cos 2\theta ,
        \end{equation*}
        which is the law of cosines for a triangle of sides $L_1$ and $a+L_1$ enclosing the angle $\pi - 2\theta$. The law of cosines does govern this construction, but with the angle $2\theta$ and not $\theta$, and it emerges by itself rather than having to be guessed. As a check, setting $\theta = 0$ leaves $\mathcal{S}_1' = (a+2L_1,\,0)$, the image sitting on the axis of the arm at a distance $a+2L_1$ from the origin, which is precisely the source retreated by twice the distance it stands from the mirror, as a mirror facing the beam squarely must do.

        The second reflection is in the beamsplitter, and here the algebra becomes remarkably simple. For an arbitrary point $\vec{X} = \left(X_1,\,X_2\right)$ the signed distance to the splitting plane is $\vec{X}\cdot\hat{n}_B = \left(X_1-X_2\right)/\sqrt{2}$, so that \ref{eq:MichReflectOp} gives

        \begin{align*}
            & \mathcal{R}_B\left(\vec{X}\right) = \left(X_1,\,X_2\right) - \frac{2\left(X_1-X_2\right)}{\sqrt{2}}\cdot\frac{1}{\sqrt{2}}\left(1,\,-1\right) = \left(X_1,\,X_2\right) - \left(X_1-X_2\right)\left(1,\,-1\right) = \\
            & \left({\color{red}X_1} - {\color{red}X_1} + X_2,\;\; {\color{red}X_2} + X_1 - {\color{red}X_2}\right) = \left(X_2,\,X_1\right),
        \end{align*}
        the terms in red cancelling in each component, so that

        \begin{equation}\label{eq:MichBSswap}
            \mathcal{R}_B\left(X_1,\,X_2\right) = \left(X_2,\,X_1\right),
        \end{equation}
        that is, \textit{reflection in the beamsplitter is nothing but the interchange of the two coordinates}. This is the analytic expression of the fact that a mirror at $45$ degrees exchanges the two arms of the instrument, sending the axis of the first onto the axis of the second, and it is the single feature that makes the Michelson interferometer so symmetric. Applying \ref{eq:MichBSswap} to \ref{eq:MichS1prime} we obtain the virtual source of the first arm,

        \begin{equation}\label{eq:MichS1}
            \mathcal{S}_1 = \left(\left(a+L_1\right)\sin 2\theta,\;\; L_1 + \left(a+L_1\right)\cos 2\theta\right),
        \end{equation}
        and the construction that leads to it is drawn in figure \ref{fig:MichelsonImages1}, where the perpendicular dropped from each point to the corresponding plane and the equality of the two distances on either side of it are marked explicitly.

        \begin{figure}[!ht]
            \centering
            \includegraphics[width=0.78\textwidth]{Chapter 4/Figures/MichelsonImages1.pdf}
            \caption{Construction of the virtual source of the first arm. The source $\mathcal{S}$ is first reflected in the plane of the mirror $M_1$, giving $\mathcal{S}_1'$, and this image is then reflected in the plane of the beamsplitter, giving $\mathcal{S}_1$. In each step the segment joining a point to its image is perpendicular to the corresponding plane and is bisected by it, which is what the right angles and the equal marks record. The interferometer of figure \ref{fig:MichelsonInterferometer} is kept faded in the background so that the construction may be followed against the real geometry of the instrument.}
            \label{fig:MichelsonImages1}
        \end{figure}
        ...
        The first reflection is in the beamsplitter, and \ref{eq:MichBSswap} disposes of it immediately,

        \begin{equation}\label{eq:MichS2prime}
            \mathcal{S}_2' = \mathcal{R}_B\left(-a,\,0\right) = \left(0,\,-a\right).
        \end{equation}
        ...
        The second reflection is in $M_2$. Using \ref{eq:MichM2} and \ref{eq:MichS2prime},

        \begin{equation*}
            \mathcal{S}_2'-\vec{P}_2 = \left(0,\,-a-L_2\right), \qquad
            \left(\mathcal{S}_2'-\vec{P}_2\right)\cdot\hat{n}_2 = -\left(a+L_2\right)\cos\phi ,
        \end{equation*}
        and substituting into \ref{eq:MichReflectOp},

        \begin{align*}
            & \mathcal{S}_2 = \left(0,\,-a\right) + 2\left(a+L_2\right)\cos\phi\,\left(-\sin\phi,\,\cos\phi\right) = \\
            & \left(-2\left(a+L_2\right)\sin\phi\cos\phi,\;\; -a + 2\left(a+L_2\right)\cos^{2}\phi\right),
        \end{align*}
        ...
        \begin{equation}\label{eq:MichS2}
            \mathcal{S}_2 = \left(-\left(a+L_2\right)\sin 2\phi,\;\; L_2 + \left(a+L_2\right)\cos 2\phi\right),
        \end{equation}
        whose construction is drawn in figure \ref{fig:MichelsonImages2}.

        \begin{figure}[!ht]
            \centering
            \includegraphics[width=0.60\textwidth]{Chapter 4/Figures/MichelsonImages2.pdf}
            \caption{Construction of the virtual source of the second arm. Here the order is reversed with respect to the first arm, since the light meets the beamsplitter before the mirror. The source is first reflected in the plane of the splitter, giving $\mathcal{S}_2'$ on the axis of the second arm at a distance $a$ below the origin, and this image is then reflected in the plane of the mirror $M_2$, giving $\mathcal{S}_2$.}
            \label{fig:MichelsonImages2}
        \end{figure}

        The two results \ref{eq:MichS1} and \ref{eq:MichS2} are the complete solution of the interferometer, and writing them together and restoring the lengths \ref{eq:MichLengths},

        \begin{align}
            & \mathcal{S}_1 = \left(\left(a+d+\ell_1\right)\sin 2\theta,\;\;\; \left(d+\ell_1\right) + \left(a+d+\ell_1\right)\cos 2\theta\right),\label{eq:MichFinal1}\\
            & \mathcal{S}_2 = \left(-\left(a+d+\ell_2\right)\sin 2\phi,\;\; \left(d+\ell_2\right) + \left(a+d+\ell_2\right)\cos 2\phi\right).\label{eq:MichFinal2}
        \end{align}

        Two checks confirm that these expressions are right. The first is a symmetry. The interchange $x \leftrightarrow y$ together with $\theta \leftrightarrow \phi$ and $\ell_1 \leftrightarrow \ell_2$ carries \ref{eq:MichFinal1} into \ref{eq:MichFinal2} up to the sign of the first component, and that is exactly as it should be, because by \ref{eq:MichBSswap} the interchange of the coordinates \textit{is} the reflection in the beamsplitter, which is precisely the operation that exchanges the two arms of the instrument. A formula that failed this test would be describing an asymmetric interferometer. The second check is the aligned instrument. Setting $\theta = \phi = 0$ both cosines become unity and both sines vanish, leaving

        \begin{equation}\label{eq:MichAligned}
            \mathcal{S}_1 = \left(0,\;a+2\left(d+\ell_1\right)\right), \qquad \mathcal{S}_2 = \left(0,\;a+2\left(d+\ell_2\right)\right),
        \end{equation}
        so that both virtual sources fall on the axis of the output arm, one behind the other, separated by

        \begin{equation*}
            \left|\left|\mathcal{S}_1-\mathcal{S}_2\right|\right| = 2\left(\ell_1-\ell_2\right),
        \end{equation*}
        ...
        \begin{equation}\label{eq:MichDelta}
            \vec{\Delta} = \mathcal{S}_1 - \mathcal{S}_2 = \left(\Delta_\perp,\;\Delta_\parallel\right),
        \end{equation}
        ...
        \begin{align}
            & \Delta_\perp = \left(a+d+\ell_1\right)\sin 2\theta + \left(a+d+\ell_2\right)\sin 2\phi,\label{eq:MichDeltaPerp}\\
            & \Delta_\parallel = \left(\ell_1-\ell_2\right) + \left(a+d+\ell_1\right)\cos 2\theta - \left(a+d+\ell_2\right)\cos 2\phi.\label{eq:MichDeltaPar}
        \end{align}
        This separation of $\vec{\Delta}$ into two pieces is not a formality, because the two components produce interference patterns of entirely different appearance, as we are about to see. It is worth insisting that no symmetry relates $\mathcal{S}_1$ to $\mathcal{S}_2$. The two sources do not lie at the same height above the splitter, since that would require the right-hand side of \ref{eq:MichDeltaPar} to vanish, nor at the same distance from the origin, since $\left|\left|\mathcal{S}_1\right|\right|$ and $\left|\left|\mathcal{S}_2\right|\right|$ are given by two laws of cosines with different sides and different angles. Each arm carries its own length and its own tilt into the answer, and the two only conspire in the particular configurations in which one of the components of $\vec{\Delta}$ happens to cancel. The complete construction, with both arms solved and the two equivalent virtual sources shown together with their separation, is collected in figure \ref{fig:MichelsonImages3}.

        \begin{figure}[!ht]
            \centering
            \includegraphics[width=0.76\textwidth]{Chapter 4/Figures/MichelsonImages3.pdf}
            \caption{The interferometer completely solved by the method of images. The construction of both arms is superposed on the faded instrument, and the two resulting virtual sources $\mathcal{S}_1$ and $\mathcal{S}_2$ are shown with their position vectors measured from the origin $O$ at the centre of the beamsplitter, together with the separation $\vec{\Delta}$ that governs the interference, resolved into the transverse and longitudinal components $\Delta_\perp$ and $\Delta_\parallel$ of \ref{eq:MichDeltaPerp} and \ref{eq:MichDeltaPar}. The configuration drawn is the generic one, with unequal arms and unequal tilts, so that the two sources are neither at the same height nor at the same distance from $O$ and both components of $\vec{\Delta}$ are present at once. From the point of view of the output, the whole interferometer has been replaced by these two mutually coherent point sources.}
            \label{fig:MichelsonImages3}
        \end{figure}
        ...
        Consider first the case in which the instrument is illuminated by \textbf{plane waves}, which is what happens when the source is placed at the focus of a collimating lens or, equivalently, when it is pushed to infinity. There are then no point sources to speak of, but the method of images applies just as well to the direction of propagation as to the position of the source, because the reflection of a wave vector in a plane obeys the very same operator \ref{eq:MichReflectOp} with the point $\vec{P}$ removed, that is $\vec{k} \to \vec{k} - 2\left(\vec{k}\cdot\hat{n}\right)\hat{n}$. Let the wave arrive along the first arm, $\vec{k} = \kappa\hat{x}$. In the first arm it is reflected in $M_1$, for which $\vec{k}\cdot\hat{n}_1 = \kappa\cos\theta$ and therefore

        \begin{equation*}
            \vec{k} \longrightarrow \kappa\left(1-2\cos^{2}\theta,\;-2\sin\theta\cos\theta\right) = -\kappa\left(\cos 2\theta,\;\sin 2\theta\right),
        \end{equation*}
        and it is then reflected in the splitter, which by \ref{eq:MichBSswap} merely interchanges the components, so that the wave leaving the first arm has

        \begin{equation}\label{eq:MichK1}
            \vec{k}_1 = -\kappa\left(\sin 2\theta,\;\cos 2\theta\right).
        \end{equation}
        ...
        \begin{equation*}
            \kappa\left(0,\,1\right) \longrightarrow \kappa\left(2\sin\phi\cos\phi,\;1-2\cos^{2}\phi\right) = \kappa\left(\sin 2\phi,\;-\cos 2\phi\right),
        \end{equation*}
        and the final transmission leaves it untouched, so that

        \begin{equation}\label{eq:MichK2}
            \vec{k}_2 = \kappa\left(\sin 2\phi,\;-\cos 2\phi\right).
        \end{equation}
        Both \ref{eq:MichK1} and \ref{eq:MichK2} reduce to $-\kappa\hat{y}$ when the mirrors are aligned, confirming that the two waves then leave the instrument along the same downward axis. The angle $\alpha$ between them follows from their scalar product,

        \begin{equation*}
            \frac{\vec{k}_1\cdot\vec{k}_2}{\kappa^{2}} = -\sin 2\theta\sin 2\phi + \cos 2\theta\cos 2\phi = \cos\left(2\theta+2\phi\right),
        \end{equation*}
        so that, remarkably,

        \begin{equation}\label{eq:MichAlpha}
            \alpha = 2\left(\theta+\phi\right),
        \end{equation}
        ...
        \begin{equation}\label{eq:MichLambdaPlane}
            \Lambda = \frac{2\pi}{K} = \frac{\lambda}{2\sin\left(\theta+\phi\right)},
        \end{equation}
        with the irradiance

        \begin{equation}\label{eq:MichIPlane}
            I = 2I_0\left[1+\cos\left(\frac{2\pi}{\Lambda}X + \frac{4\pi}{\lambda}\left(\ell_1-\ell_2\right) + \delta_B\right)\right],
        \end{equation}
        where $X$ is measured on the observation plane along the direction of $\vec{K}$, the term $4\pi(\ell_1-\ell_2)/\lambda$ is the phase accumulated by the difference $2(\ell_1-\ell_2)$ of the two optical paths on the axis, and $\delta_B$ is the phase introduced by the splitter itself, equal to $\pi$ for a symmetric beamsplitter, which is what makes the two output ports of the instrument complementary and conserves the energy between them. The behaviour of \ref{eq:MichLambdaPlane} is the expected one, since aligning the mirrors sends $\theta+\phi \to 0$ and therefore $\Lambda \to \infty$, the fringes widening without limit until the whole field is uniformly bright or uniformly dark according to the value of the arm difference, which is exactly what one observes when a Michelson interferometer is well aligned under collimated illumination.

        Consider now the case of \textbf{spherical waves}, that is, of a genuine point source at a finite distance, which is the case the whole image construction was built for. The instrument has been replaced by the two mutually coherent point sources $\mathcal{S}_1$ and $\mathcal{S}_2$ of \ref{eq:MichFinal1} and \ref{eq:MichFinal2}, and we observe the light on a screen placed perpendicular to the output axis at a distance $D$ below the beamsplitter, with coordinates $X$ in the plane of the figure and $Z$ perpendicular to it. Writing $\mathcal{S}_j = (X_j, Y_j)$ the distances from each source to a point of the screen are

        \begin{equation*}
            r_j = \left[\left(X-X_j\right)^{2} + Z^{2} + \left(D+Y_j\right)^{2}\right]^{1/2},
        \end{equation*}
        ...
        \begin{equation*}
            r_j \approx \left(D+Y_j\right) + \frac{\left(X-X_j\right)^{2}+Z^{2}}{2\left(D+Y_j\right)} .
        \end{equation*}
        ...
        \begin{align*}
            & r_1 - r_2 \approx \left(Y_1-Y_2\right) + \frac{\left(X-X_1\right)^{2} + {\color{red}Z^{2}} - \left(X-X_2\right)^{2} - {\color{red}Z^{2}}}{2\bar{D}} = \\
            & \Delta_\parallel + \frac{{\color{red}X^{2}} - 2XX_1 + X_1^{2} - {\color{red}X^{2}} + 2XX_2 - X_2^{2}}{2\bar{D}} = \Delta_\parallel - \frac{\Delta_\perp}{\bar{D}}X + \frac{X_1^{2}-X_2^{2}}{2\bar{D}},
        \end{align*}
        ...
        \begin{equation}\label{eq:MichPhaseSph}
            \triangle\phi = \kappa\left(r_2-r_1\right) + \delta_B = \frac{2\pi}{\lambda}\frac{\Delta_\perp}{\bar{D}}X - \frac{2\pi}{\lambda}\Delta_\parallel + \delta_B,
        \end{equation}
        and the irradiance on the screen is

        \begin{equation}\label{eq:MichISph}
            I = 2I_0\left[1+\cos\left(\frac{2\pi}{\lambda}\frac{\Delta_\perp}{\bar{D}}X - \frac{2\pi}{\lambda}\Delta_\parallel + \delta_B\right)\right].
        \end{equation}

        Equation \ref{eq:MichISph} shows at once the different roles of the two components of the separation. The transverse component $\Delta_\perp$ is the only one that depends on the position on the screen, and it produces straight fringes perpendicular to $X$, that is, parallel to the direction $Z$, with the spacing

        \begin{equation}\label{eq:MichLambdaSph}
            \Lambda = \frac{\lambda\bar{D}}{\Delta_\perp} = \frac{\lambda\bar{D}}{\left(a+d+\ell_1\right)\sin 2\theta + \left(a+d+\ell_2\right)\sin 2\phi},
        \end{equation}
        ...
        That last statement is true only within the paraxial expansion, and abandoning it recovers the most familiar pattern of the instrument. When the mirrors are aligned we have $\Delta_\perp = 0$ and \ref{eq:MichLambdaSph} gives an infinite spacing, meaning that no straight fringes exist, so the pattern must be sought at angles large enough for the expansion to fail. Two point sources separated by $\Delta_\parallel$ along the line of sight, observed in the direction making an angle $\psi$ with that line, deliver a path difference equal to the projection $\Delta_\parallel\cos\psi$, and the condition for a bright fringe is therefore

        \begin{equation}\label{eq:MichRings}
            \Delta_\parallel\cos\psi = m\lambda, \qquad m \in \mathbb{Z},
        \end{equation}
        which describes circles centred on the output axis, since all the directions sharing one value of $\psi$ form a cone. These are the fringes of equal inclination, the celebrated circular fringes of the Michelson interferometer, and with the aligned value $\Delta_\parallel = 2(\ell_1-\ell_2)$ the condition \ref{eq:MichRings} becomes

        \begin{equation}\label{eq:MichRingsAligned}
            2\left(\ell_1-\ell_2\right)\cos\psi = m\lambda,
        \end{equation}
